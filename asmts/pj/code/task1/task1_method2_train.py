...
#            claim-evidence pair as pos, Negative Sampling             #
########################################################################
...
for i in range(len(train)):
    ...
    for k in range(len(train)):
        if i == k : continue
        for j in range(len(train['evidences'][k])):
            train_examples.append(InputExample(texts=[train['claim_text'][i], evidence[0][train['evidences'][k][j]]], label=NEG_SCORE))
...
BATCH_SIZE = 128    # NOTE: batch_size 越大越快；256 超出cuda memory
EPOCHS = 4
