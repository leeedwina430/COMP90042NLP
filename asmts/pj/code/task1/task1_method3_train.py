...
#           All other claim-evidence pair as neg            #
#############################################################
...
# save model
model.save(f'model/claim_evidence_bert_{METHOD}_cased_{EPOCHS}')
print('model saved!')



start = time.time()
claim_embedding = model.encode(val['claim_text'], batch_size=256, 
                                convert_to_tensor=True)
evidence_embeddings = model.encode(evidence[0], batch_size=BATCH_SIZE,
                                convert_to_tensor=True)
print('encode data: ', time.time() - start, ' s')

similarity_scores = torch.cosine_similarity(claim_embedding, evidence_embeddings)

k = 3
for i in range(len(val)):
    top_k_indices = similarity_scores[i].argsort(descending=True)[:k]
    top_k_evidence = [evidence[0].iloc[i] for i in top_k_indices]
    val['top_k_evidence'][i] = top_k_evidence

val.to_json(f'output/eval_{METHOD}_cased_{EPOCHS}.json', orient='index')
